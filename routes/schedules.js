...
router.get('/new', authenticationEnsure, csrfProtection, (req, res, next) => {
...
router.get('/:scheduleId', authenticationEnsure, (req, res, next) => {
  ...
  Schedule.findOne({
    ...
    .then(availabilities => {
      ...
      // 全ユーザー、全候補で二重ループしてそれぞれの出欠がない場合には、「欠席」を設定する
...
router.get('/:scheduleId/edit', authenticationEnsure, csrfProtection, (req, res, next) => {
  Schedule.findOne({
    where: {
      scheduleId: req.params.scheduleId
    }
...
router.post('/:scheduleId', authenticationEnsure, csrfProtection, (req, res, next) => {
  Schedule.findOne({
    where: {
      scheduleId: req.params.scheduleId
    }
